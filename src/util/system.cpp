...
const char * const BITCOIN_CONF_FILENAME = "beastbucks.conf";
...
#else
    const char* pszModule = "beastbucks";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BeastBucks
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BeastBucks
    // Mac: ~/Library/Application Support/BeastBucks
    // Unix: ~/.beastbucks
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BeastBucks";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BeastBucks";
#else
    // Unix
    return pathRet / ".beastbucks";
#endif
#endif
}
