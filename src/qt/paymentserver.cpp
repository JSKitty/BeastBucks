...
const QString BITCOIN_IPC_PREFIX("beastbucks:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the beastbucks: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // beastbucks: URI
        {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click beastbucks: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start beastbucks: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling beastbucks: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("beastbucks://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'beastbucks://' is not a valid URI. Use 'beastbucks:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // beastbucks: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid BeastBucks address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
