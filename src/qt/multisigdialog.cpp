...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->mapAddressBook.count(DecodeDestination(address)))
        wallet->SetAddressBook(DecodeDestination(address), label, "");  //TODO - maybe add purpose instead of empty string?
}
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    ui->statusLabel->setText(tr("The transaction is sent to BeastBucks network."));
}
