...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BeastBucks address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || (uri.scheme() != QString("beastbucks") && uri.scheme() != QString("666")))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("beastbucks:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
