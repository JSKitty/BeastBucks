...
"""Class for beastbucksd node under test"""
...
class TestNode():
    """A class for representing a beastbucksd node under test.
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.cli = TestNodeCLI(beastbucks_cli, self.datadir)
    ...
    def __del__(self):
        # Ensure that we don't leave any beastbucksd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by beastbucksd, and
        ...
        self.running = True
        self.log.debug("beastbucksd started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the beastbucksd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'beastbucksd exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
...
class TestNodeCLI():
    """Interface to beastbucks-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.beastbuckscli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with beastbucks-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run beastbucks-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same beastbucks-cli call"
        ...
        self.log.debug("Running beastbucks-cli {}".format(p_args[2:]))
